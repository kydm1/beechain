...
func (t *tokennote) ExecTransWithdraw(accountDB *account.DB, tx *types.Transaction, action *tokenty.TokennoteAction, index int) (*types.Receipt, error) {
	tokennotelog.Error("transfer","from",tx.From(),"TO",tx.GetRealToAddr())
	if (action.Ty == tokenty.ActionTransfer) && action.GetTransfer() != nil {
		...
		tokennoteDB,err := t.getTokennoteDB(transfer.Cointoken)
		if err != nil {
			tokennotelog.Error("ExecTransWithdraw","get tokennotedb err ",err)
			return nil,err
		}
		var repay int64
		if transfer.Cointoken != tokenty.TokenCCNY {
			c := false

			for _,v := range tokennoteDB.Holds {
				if v.Addr == from && v.Status == tokenty.TokennoteStatusAgreeLoan {
					bigamount := big.NewInt(v.Amount)
					bigtamount := big.NewInt(transfer.Amount)
					bigrepay := big.NewInt(v.Repayamount)
					base := big.NewInt(int64(1))

					temp :=base.Sub(bigamount,bigtamount).Mul(base,bigrepay).Div(base,bigamount).Int64()
					//temp := int64(((v.Amount-transfer.Amount) * v.Repayamount) / v.Amount )
					repay = v.Repayamount - temp
					v.Repayamount = temp

					v.Amount -= transfer.Amount
					if v.Amount < 0 {
						return nil,tokenty.ErrTokennoteAmountLow
					}
			...
			if !c {
				return nil,tokenty.ErrTokenoteNotAvaible
			}
		}

		hold := &tokenty.TokennoteHold{
			Addr:tx.GetRealToAddr(),
			Amount:transfer.Amount,
			Repayamount:repay,
			Currency:transfer.Cointoken,
			Creator:tokennoteDB.Issuer,
			LoanTime:t.GetBlockTime(),
			Status:tokenty.TokennoteStatusAgreeLoan,
		}
		tokennoteDB.Holds = append(tokennoteDB.Holds,hold)
		key := calcTokennoteAddrNewKeyS(transfer.Cointoken,tokennoteDB.Issuer)
		keyhold := calcTokennoteHoldKeyNew(transfer.Cointoken,tx.GetRealToAddr())

		var logs []*types.ReceiptLog
		var kv []*types.KeyValue

		kv = append(kv,&types.KeyValue{Key:key,Value:types.Encode(tokennoteDB)},&types.KeyValue{Key:keyhold,Value:types.Encode(hold)})
		receipt := &types.Receipt{Ty: types.ExecOk, KV: kv, Logs: logs}
		receipt2,err := accountDB.Transfer(from, tx.GetRealToAddr(), transfer.Amount)
		if err != nil {
			tokennotelog.Error("TRANSANFER","tokennote transfer err",err)
			return nil,err
		}
	...
	} else if action.Ty == tokenty.TokennoteActionTransferToExec && action.GetTransferToExec() != nil {
...
//获取白条calcTokennoteAddrNewKeyS·
func (t *tokennote) getTokennoteDB(currncy string) (*tokenty.Tokennote, error) {
	var tokennote,tokennotedb tokenty.Tokennote
	value,err := t.GetStateDB().Get(calcTokennoteKey(currncy))
	if err != nil {
		return nil,err
	}
	err = types.Decode(value,&tokennote)
	if err != nil {
		return nil,err
	}
	value1,err := t.GetStateDB().Get(calcTokennoteAddrNewKeyS(currncy,tokennote.Issuer))
	if err != nil {
		return nil,err
	}
	err = types.Decode(value1,&tokennotedb)
	if err != nil {
		return nil,nil
	}
	return &tokennotedb,nil
}
...
func (t *tokennote) ExecLocalTransWithdraw(tx *types.Transaction, receipt *types.ReceiptData, index int) (*types.LocalDBSet, error) {
	set := &types.LocalDBSet{}
	if receipt.GetTy() != types.ExecOk {
		return set, nil
	}
	//执行成功
	var action tokenty.TokennoteAction
...
func (t *tokennote) ExecDelLocalLocalTransWithdraw(tx *types.Transaction, receipt *types.ReceiptData, index int) (*types.LocalDBSet, error) {
	set := &types.LocalDBSet{}
	if receipt.GetTy() != types.ExecOk {
		return set, nil
	}
...
func (t *tokennote) CheckTokennoteStatus(tokennote,to string,amount int64) error {
	//判断是否可以转账
	tokennotedb,err := t.getTokennoteDB(tokennote)
	if err != nil {
		return err
	}
	if tokennotedb.Status != tokenty.TokennoteStatusCreated {
		return tokenty.ErrTokenoteNotAvaible
	}
	var amount1 int64
	for _,v := range tokennotedb.Holds {
		if v.Addr == to && v.Currency == tokennote && v.Status == tokenty.TokennoteStatusAgreeLoan {
			amount1 += v.Amount
		}
	}
	if tokennote == tokenty.TokenCCNY {
		amount1 = amount
	}
	if amount1 < amount {
		return tokenty.ErrTokennoteAmountLow
	}
	return nil
}
